...
func (c commentResolver) List(ctx context.Context, article model.Article) ([]model.Comment, error) {
	...
	commentList, err := cache.QueryCaches(ctx, cache.Comment{Aid: article.Id}, func() (interface{}, error) {
		return model.CommentList(tx, article.Id)
	})
	if err != nil {
		logger.Error().Caller().Err(err).Send()
		return nil, errors.New("获取文章评论失败")
	}
	return commentList.([]model.Comment), nil
}
